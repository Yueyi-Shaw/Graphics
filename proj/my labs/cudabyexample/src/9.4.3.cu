...
{
    __shared__ unsigned int blockLocalMem[256];
    blockLocalMem[threadIdx.x] = 0;
    __syncthreads();
    ...
    while (i < size)
    {
        atomicAdd(&(blockLocalMem[buffer[i]]), 1);
        i += stride;
    }

    __syncthreads();
    atomicAdd(&(histo[threadIdx.x]), blockLocalMem[threadIdx.x]);
}
